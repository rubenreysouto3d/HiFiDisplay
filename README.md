...
During active playback, the source badge is the single entry point for source, design, palette, and diagnostics. Transport controls share its top rail and fade away after six seconds, while the source remains available. Tap the ambient display to reveal controls; using a transport control or seeking restarts the interval. Artwork, bounded metadata, times, and a thin live progress line remain visible without layout jumps. Track and artwork changes crossfade, touch feedback is restrained, missing artwork uses a palette-aware record treatment, and a minute periodic offset helps protect always-on OLED panels. Configuration remains permanently visible on permission, error, and no-session screens.
