...
Track transitions and continuous playback effects are separate preferences. `Pulse`, `Drift`, and `Halo` animate only while the active session reports playback; `Still` disables continuous motion. Their phase is derived from the session's estimated playback position, so the cadence remains stable across Compose recompositions and media callbacks. It is deliberately described as position-locked visual cadence—not audio beat detection—because Android `MediaSession` does not expose a beat or amplitude stream.
...
The palette system controls the complete material stack without changing layout: background luminance, surfaces, typography, action color, and optical contrast all travel together. `Arctic Silver` is a genuinely light cool-glass treatment, while `Champagne Frost` provides a warmer luminous alternative without using pure white. `OLED Absolute` uses an opaque `#000000` canvas and suppresses nonessential ambient fields and grids so unused pixels remain off; controls and display-specific structure appear only where they carry information. Hi-Fi Green, Warm Amber, Cobalt Night, Velvet Violet, and Ruby Signal remain the darker color families.

The display panel is split into direct `SOURCE`, `DESIGN`, `COLOR`, and `MOTION` destinations. Five designs fit in one compact design grid, all eight palettes fit in a four-by-two wide grid, and track transitions and playback presence each use compact two-by-two choices with live previews. The full set of options remains available while common changes take fewer taps and no long scrolling.
